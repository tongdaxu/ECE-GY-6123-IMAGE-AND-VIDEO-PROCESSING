...
#           nn.init.kaiming_normal_(layer.weight, mode='fan_out')
        ...
        # self.side_output_bv = nn.Conv3d(b3_c, 40, kernel_size=5, stride=2, padding=2)
        # self.side_conv_bn = nn.BatchNorm3d(40)
        # self.side_pool1 = nn.AdaptiveAvgPool3d((1,1,1))
        # self.side_pool2 = nn.AdaptiveMaxPool3d((1,1,1))
...
#         print(x.size())
        ...
        # x_side = self.relu(self.side_conv_bn(self.side_output_bv(x)))
        # x_side1 = self.side_pool1(x_side)
        # x_side2 = self.side_pool2(x_side)
        # x_linear = torch.cat((x_side1.view(-1, 40), x_side2.view(-1, 40)), 1)
        # x_bv = self.fc1(x_linear)
